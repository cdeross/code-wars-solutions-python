...
divisible by 7 is obtained; you can stop when this number has at most 2 digits because you are supposed to know if a
...
def seven(m):
    ...
    if m == 0:
        ...
        while m > 0:
            counter += 1
            last_digit = int(m_string[-1])
            front_digits = int(m_string[0:-1])
            last_digit = last_digit * 2
            m = front_digits - last_digit
            if len(str(m)) > 2:
                m_string = str(m)
            else:
                if m % 7 == 0:
