...
def nbMonths(start_price_old, start_price_new, saving_per_month, percent_loss_by_month):
    ...
    while (total_savings + old_car) < new_car:
        month += 1
        total_savings += saving_per_month
        if month % 2 == 0:
            percent_loss_by_month += 0.5
        new_car = new_car - ((percent_loss_by_month / 100) * new_car)
        old_car = old_car - ((percent_loss_by_month / 100) * old_car)
    return [month,  round((total_savings + old_car) - new_car)]
