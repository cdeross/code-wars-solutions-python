...
only the first letter of the word is in upper case) or (b) considered to be an exception and put entirely into
...
def title_case(title, minor_words=''):
    if title == "":
        ...
        sen = (title.lower()).split(" ")
        arr = []
        sentence = " "
        minor_word = (minor_words.lower()).split(" ")
        for i in sen:
            if i in minor_word:
                arr.append(i)
            else:
                arr.append(i.capitalize())
        sentence = " ".join(i for i in arr)
